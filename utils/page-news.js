...
async function fetchDetailNews() {
    try {
        ...
        try {
            ...
            // Track dan simpan semua URL gambar yang ditemukan
            let imageUrls = [];
            let imgMatches = konten.match(/https?:\/\/[^\s<>]+\.(png|jpg|jpeg|gif|webp)(\b|$)/gi);

            if (imgMatches) {
                imageUrls = [...new Set(imgMatches)]; // Hapus duplikat
            }

            // Jika ada gambar, buat gallery di atas konten
            let galleryHTML = '';
            if (imageUrls.length > 0) {
                if (imageUrls.length === 1) {
                    // Jika hanya satu gambar, tampilkan dengan ukuran penuh
                    galleryHTML = `
            <div class="mb-6">
              <img src="${imageUrls[0]}" alt="News Image" class="max-w-full rounded-lg mx-auto">
            </div>
          `;
                } else {
                    // Jika lebih dari satu gambar, buat gallery
                    galleryHTML = '<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">';
                    imageUrls.forEach(url => {
                        galleryHTML += `
              <div class="aspect-square overflow-hidden rounded-lg">
                <img src="${url}" alt="News Image" class="w-full h-full object-cover">
              </div>
            `;
                    });
                    galleryHTML += '</div>';
                ...
                // Hapus URL gambar dari konten karena sudah ditampilkan di gallery
                imageUrls.forEach(url => {
                    konten = konten.replace(url, '');
                });
            }

            // Konversi links dalam tanda kurung menjadi links yang bisa diklik
            konten = konten.replace(/\(https?:\/\/[^\s<>)]+[^\s<>)]*\)/g, (match) => {
                const url = match.substring(1, match.length - 1);
                return `(<a href="${url}" target="_blank" class="text-blue-400 underline">${url}</a>)`;
            });

            // Reduce excessive spaces (multiple newlines)
            konten = konten.replace(/\n\s*\n\s*\n/g, "\n\n");

            // Bersihkan spasi berlebih akibat penghapusan URL gambar
            konten = konten.replace(/\s{2,}/g, ' ').trim();

            // Format bullet points with member names to make them stand out
            konten = konten.replace(/●\s+(.*?)Tanggal:/g, (match, memberName) => {
                return `● <strong>${memberName.trim()}</strong> Tanggal:`;
            });

            // Ganti newline dengan <br> dan format tautan yang belum diformat
            konten = konten.replace(/\n/g, "<br>")
                .replace(/(https?:\/\/[^\s<>]+\.(?:com|id|net|org)[^\s<>]*)/g, (match) => {
                    // Skip URLs that are already part of image tags or other links
                    if (konten.includes(`<img src="${match}"`) ||
                        konten.includes(`href="${match}"`) ||
                        match.match(/\.(png|jpg|jpeg|gif|webp)(\b|$)/i)) {
                        return match;
                    }
      ...
      `;
            ...
            // Make sure all elements have white text except links
            const kontenElement = container.querySelector('.text-white.leading-relaxed');
            if (kontenElement) {
                kontenElement.querySelectorAll('*:not(a):not(strong)').forEach(el => {
                    el.style.color = 'white';
                });
                // Make strong elements stand out more
                kontenElement.querySelectorAll('strong').forEach(el => {
